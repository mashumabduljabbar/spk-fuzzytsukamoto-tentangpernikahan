...
class Adm extends CI_Controller {
	...
	public function h_angket() {
		...
		if ($uri3 == "det") {
			$a['detil_tes'] = $this->db->query("SELECT m_mapel.nama AS namaMapel, m_guru.nama AS nama_guru, 
												...
												INNER JOIN m_guru ON tr_guru_tes.id_guru = m_guru.id
												WHERE tr_guru_tes.id = '$uri4'")->row();
			//$a['statistik'] = $this->db->query("SELECT MAX(nilai) AS max_, MIN(nilai) AS min_, AVG(nilai) AS avg_  FROM tr_angket WHERE tr_angket.id = '$uri4'")->row();

			$a['hasil'] = $this->db->query("SELECT m_guru.nama, tr_angket.nilai
											FROM tr_angket
											INNER JOIN m_guru ON tr_angket.id_guru = m_guru.id
											WHERE tr_angket.id = '$uri4'")->result();
			$a['p'] = "m_guru_tes_hasil_detil_angket";
			//echo $this->db->last_query();
		} else {
			$a['p']	= "m_guru_tes_hasil_angket";
		}
	...
	public function ikuti_ujian() {
		...
		$a['data'] = $this->db->query("SELECT tr_guru_tes.*, m_mapel.nama AS mapel,
										...
										(SELECT hasil_ujian FROM tr_ikut_ujian WHERE tr_ikut_ujian.id_user = ".$a['sess_konid']." AND tr_ikut_ujian.id_tes = tr_guru_tes.id) AS hasil_ujian
										FROM tr_guru_tes
										INNER JOIN m_mapel ON tr_guru_tes.id_mapel = m_mapel.id
	...
	public function ikut_ujian() {
		...
		$bobot_maksimum = "100";
		$penilaian_minimum = "0";
		$penilaian_maksimum = "3";

		//HASIL UJIAN
		$nilai_ujian = $nilai;
		$bobot_ujian = $jumlah_bobot;

		$nol = "0";
		$satu = "1";

		$vMin[0] = $nilai_minimum;
		$vMin[1] = $bobot_minimum;
		$vMin[2] = $penilaian_minimum;
		$vMax[0] = $nilai_maksimum;
		$vMax[1] = $bobot_minimum;
		$vMax[2] = $penilaian_maksimum;
		...
		/* END TSUKAMOTO*/
			...
			$this->db->query("UPDATE tr_ikut_ujian SET jml_benar = ".$jumlah_benar.", nilai_bobot = ".$jumlah_bobot.", nilai = '".$nilai."', list_jawaban = '".$update_."', status = 'N', hasil_ujian = '$hasil_ujian' WHERE id_tes = '$uri4' AND id_user = '".$a['sess_konid']."'");
			$a['status'] = "ok";
			$this->j($a);
			exit;		
		} else {
	...
	public function angket_penilaian() {
		$this->cek_aktif();
		...
		//echo $this->db->last_query();
		session_start();
		$_SESSION['sess_konid'] = $a['sess_konid'];
		$a['p']	= "m_angket";
		$this->load->view('aaa', $a);
	}
	
	public function ikut_angket() {
		$this->cek_aktif();
		...
		$this->load->view('aaa_ikut_angket');
		
		

	}
	
	public function m_soal_angket() {
		$this->cek_aktif();
	...
	public function createPost(){
		//Insert dengan Controller dari Post
		$id_guru = $this->input->post('id_guru');
		$id_mapel = $this->input->post('id_mapel');
		$id_user = $this->input->post('id_user');
		$jumlah_angket = $this->input->post('jumlah_angket');
		$jml = 0;
		$jumlah_record_id = count($_POST['id']);
		for($v=0; $v<=($jumlah_record_id-1); $v++){
			$id = $_POST['id'][$v];
			$jawaban[$v] = $_POST['jawaban'.$id];
			$array_jawaban = implode(",",$jawaban);
			$jml += $jawaban[$v];
		}
		//echo $jml*5;
		//$jumlah_jawaban = count($jawaban);
		$nilai_angket = ($jml/($jumlah_angket*4))*100; //(20/(5*4))*100
		...
		$this->db->insert("tr_angket", $data);
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/angket_penilaian'>";
		
	}	
	
	public function createAngket(){
		//Insert dengan Controller dari Post
		$soal = $this->input->post('soal');
		$id_mapel = $this->input->post('id_mapel');
		$id_guru = mysql_fetch_array(mysql_query("select id_guru from tr_guru_mapel where id_mapel='$id_mapel'"));
		...
		$this->db->insert("m_soal_angket", $data);
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/m_soal_angket/$id_mapel'>";
		
	}	
	
	public function updateAngket(){
		$id = $this->input->post('id');
		$soal = $this->input->post('soal');
		$mapel = $this->input->post('mapel');
		$data = array('soal' => $soal);
		$this->db->where('id', $id);
		$this->db->update('m_soal_angket', $data);
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/m_soal_angket/$mapel'>";
	}
	
	public function deleteAngket($id = '', $mapel = ''){
		$this->db->where('id', $id);
		$this->db->delete('m_soal_angket'); 
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/m_soal_angket/$mapel'>";
	}	
	...
	public function ikut_tanya_jawab() {
		$this->cek_aktif();
		...
		$this->load->view('aaa_ikut_tanya_jawab');
	}
	
	public function createTanyajawab(){
		$soal = $this->input->post('soal');
		$id_mapel = $this->input->post('id_mapel');
		$id_guru = $this->input->post('id_guru');
		$id_user = $this->input->post('id_user');
		...
		$this->db->insert("m_soal_tanya_jawab", $data);
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/ikut_tanya_jawab/$id_guru/$id_mapel/$id_user'>";
		
	}
	
	
	public function updateTanyajawab(){
		...
		$this->db->where('id', $id);
		$this->db->update('m_soal_tanya_jawab', $data);
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/ikut_tanya_jawab/$id/$id_guru/$id_mapel/$id_user'>";
	}
	
	public function deleteTanyajawab($id = '', $id_guru = '', $id_mapel = '', $id_user = ''){
		$this->db->where('id', $id);
		$this->db->delete('m_soal_tanya_jawab'); 
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/ikut_tanya_jawab/$id_guru/$id_mapel/$id_user'>";
	}
	
	public function tanya_jawab_guest() {
		$this->cek_aktif();
	...
	public function sudah_selesai_ujian() {
		...
		$q_nilai = $this->db->query("SELECT hasil_ujian, tgl_selesai FROM tr_ikut_ujian WHERE id_tes = $uri3 AND id_user = '".$a['sess_konid']."' AND status = 'N'")->row();
		if (empty($q_nilai)) {
			redirect('adm/ikut_ujian/_/'.$uri3);
		} else {
			$a['p'] = "v_selesai_ujian";
			$a['data'] = "<div class='alert alert-danger'>Anda telah selesai mengikuti ujian ini pada : <strong style='font-size: 16px'>".tjs($q_nilai->tgl_selesai, "l")."</strong>, dengan hasil ujian : <strong style='font-size: 16px'>".$q_nilai->hasil_ujian."</strong></div>";
		}

		$this->load->view('aaa', $a);
	}
	
	public function sudah_selesai_angket() {
		$this->cek_aktif();
		...
		$q_nilai = $this->db->query("SELECT nilai, tgl_selesai FROM tr_angket WHERE id_tes = $uri3 AND id_user = '".$a['sess_konid']."' AND status = 'N'")->row();
		if (empty($q_nilai)) {
			redirect('adm/ikut_angket/_/'.$uri3);
		} else {
			$a['p'] = "v_selesai_ujian";
	...
	public function suksesguest() {
		$this->load->view('halamanutama');
	}
	public function hasilguest() {
		$this->load->view('halamanutama');
	}
	
	public function readGuest() {
		$userid_guest = $this->input->post('userid_guest');
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/hasilguest/$userid_guest'>";
	}
	
	public function createGuest(){
		//Insert dengan Controller dari Post
		$userid_guest = $this->input->post('userid_guest');
		$tanya_guest = $this->input->post('tanya_guest');
		$created_guest = date("Y-m-d H:i:s");
		$data = array('id_guest' => '',
			'userid_guest' => $userid_guest,
			'tanya_guest' => $tanya_guest,
			'created_guest' => $created_guest
		);
		
		$this->db->insert("tr_guest", $data);
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/suksesguest/$userid_guest'>";
		
	}
	public function updateGuest(){
		$id_guest = $this->input->post('id_guest');
		$jawab_guest = $this->input->post('jawab_guest');
		$penjawab_guest = $this->input->post('penjawab_guest');
		$update_guest = date("Y-m-d H:i:s");
		$data = array(
			'jawab_guest' => $jawab_guest,
			'penjawab_guest' => $penjawab_guest,
			'update_guest' => $update_guest
		);
		$this->db->where('id_guest', $id_guest);
		$this->db->update('tr_guest', $data);
		echo "<meta http-equiv='refresh' content='0;url=".base_url()."adm/tanya_jawab_guest/'>";
	}	
