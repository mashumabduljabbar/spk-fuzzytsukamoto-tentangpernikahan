...
function m_soal_angket_e(id) {
	$("#m_soal_angket").modal('show');
	$.ajax({
		type: "GET",
		url: base_url+"adm/m_soal_angket/det/"+id,
		success: function(data) {
			$("#id").val(data.id);
			//$("#id_mapel").val(data.id_mapel);
			$("#bobot").val(data.bobot);
...
function m_soal_angket_s() {
	//e.preventDefault();
	var f_asal	= $("#f_soal");
	var form	= getFormData(f_asal);
	$.ajaxFileUpload({
		url             : base_url + 'adm/m_soal_angket/simpan/', 
		secureuri       : false,
		fileElementId   : 'gambar_soal',
		data 			: form,
		dataType		: 'jsonp',
		...
		success : function (data) {
			...
			if(d.status == 'ok') {
				$('#konfirmasi').html('<div class="alert alert-success">'+d.msg+'</div>');			
				window.location.assign(base_url+"adm/m_soal_angket/pilih_mapel/"+d.id_mapel);
			} else {
				$('#konfirmasi').html('<div class="alert alert-danger">gagal</div>');
			}
		}
	...
	$.ajax({		
		...
		if (response.status == "ok") {
			window.location.assign(base_url+"adm/m_soal_angket"); 
		} else {
			console.log('gagal');
		}
	});
...
function m_soal_angket_h(id) {
	if (confirm('Anda yakin..?')) {
		$.ajax({
			type: "GET",
			url: base_url+"adm/m_soal_angket/hapus/"+id,
			success: function(response) {
				if (response.status == "ok") {
					window.location.assign(base_url+"adm/m_soal_angket"); 
				} else {
